...
def especimen_mas_inclinado(lista_arboles):
    ...
    lista_inclinacion = []
    lista_tpl = []
    for fila in lista_especies:
        lista_inclinacion = obtener_inclinaciones(lista_arboles, fila)
        mayor = max(lista_inclinacion)
        tpl = (fila, mayor)
        lista_tpl.append(tpl)
    mayor_inclinacion = max(lista_tpl, key=itemgetter(1))[0]
    inclinacion = max(lista_tpl, key=itemgetter(1))[1]
    return mayor_inclinacion, inclinacion
...
def especie_promedio_mas_inclinada(lista_arboles):
    ...
    for fila in lista_especies:
        lista_inclinacion = obtener_inclinaciones(lista_arboles, fila)
        promedio = round(sum(lista_inclinacion) / len(lista_inclinacion), 2)
        tpl = (fila, promedio)
        lista_tpl.append(tpl)
    mayor_inclinacion = max(lista_tpl, key=itemgetter(1))[0]
    inclinacion = max(lista_tpl, key=itemgetter(1))[1]
